...
def play_hand(hand, word_list):
	...
	* The hand finishes when there are no more unused letters.
	...
	display_hand(hand)
	word = input("Enter word, or '!!' to indicate that you're finished: ")
	
	while word != '!!':
		print('\n')
		if is_valid_word(word, hand, word_list):
			total_score += get_word_score(word, calculate_handlen(hand))
			print('"' + word + '" ' + 'earned ' + str(get_word_score(word, calculate_handlen(hand))) + ' points. Total ' + str(total_score) + ' points.')
		else:
...
def play_game(word_list):
	...
	* Returns the total score for the series of hands
	...
	while rounds > 0:
		...
		str.lower(sub)
		if sub == 'yes':
			letter = str(input('Enter a letter to substitute: '))
